...
namespace PotatoFinch.TmgDotsJam.Common {
    ...
    [BurstCompile]
    partial class GameObjectCompanionSystem : SystemBase
    {
        ...
        protected override void OnCreate()
        {
            ...
            m_CreatedQuery = new EntityQueryBuilder(Allocator.Temp)
                .WithAll<GameObjectCompanionLink>()
            ...
            m_DestroyedQuery = new EntityQueryBuilder(Allocator.Temp)
                ...
                .WithNone<GameObjectCompanionLink>()
                .Build(this);
            m_ModifiedQuery = new EntityQueryBuilder(Allocator.Temp)
                .WithAll<GameObjectCompanionLink>()
                .Build(this);
            m_ModifiedQuery.SetChangedVersionFilter(typeof(GameObjectCompanionLink));
        }
        ...
        protected override void OnUpdate()
        {
            using (s_ProfilerMarkerAddNew.Auto())
            {
                if (!m_CreatedQuery.IsEmpty)
                {
                    ...
                    {
                        var entity = entities[i];
                        var link = EntityManager.GetComponentData<GameObjectCompanionLink>(entity);
            ...
            {
                foreach (var (link, entity) in SystemAPI.Query<GameObjectCompanionLink>().WithChangeFilter<GameObjectCompanionLink>().WithEntityAccess())
                {
